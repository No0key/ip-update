...
if [[ "${DIG_RETURN_CODE}" -ne 0 || "${DIG_STATUS}" != "status: NOERROR" ]]; then
	log "Dig finished with code $DIG_RETURN_CODE and $DIG_STATUS"
	DIG_CHECK_STATUS="false"
	echo $DIG_CHECK_STATUS
	exit 1
elif [[ $(echo "${DIG_INPUT}" | awk '(/^[^;;]/) {print $NF}' | wc -l) -eq 0 ]]; then
	log "Dig finished with code "${DIG_RETURN_CODE}" and "${DIG_STATUS}", received empty list of NS servers"
	DIG_CHECK_STATUS="false"
	echo $DIG_CHECK_STATUS
	exit 1
else
...
get_main_dns () {
DIG_CHECK_STATUS=$(dig_checker "$(dig $1 ns)")
if (( $(echo ${DIG_CHECK_STATUS} | wc -c) == 5 )); then
	echo "+++++++++++++++++++++++FALSE+++++++++++++++++++++"
	exit 1
else
	MAIN_DNS=$(echo "${DIG_CHECK_STATUS}" | grep -Eo ' [A-Za-z0-9]\.[A-Z0-9a-z]+?\.[A-Za-z0-9]+?\.')
